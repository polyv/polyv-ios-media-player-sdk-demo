...
static NSString *const PLVVodMediaSubtitleErrorDomain = @"net.polyv.subtitle.error";
...
@interface PLVVodMediaSubtitleParser ()

@property (nonatomic, strong) NSMutableArray<PLVVodMediaSubtitleItem *> *subtitleItems;
@property (nonatomic, strong) NSMutableArray<PLVVodMediaSubtitleItem *> *subtitleAtTopItems;

@property (nonatomic, strong) NSDictionary<NSNumber *, PLVVodMediaSubtitleItem *> *subtitleItemsDictionary;

@end

@implementation PLVVodMediaSubtitleParser

#pragma mark - property

- (NSMutableArray<PLVVodMediaSubtitleItem *> *)subtitleItems {
    if (!_subtitleItems) {
...
- (NSMutableArray<PLVVodMediaSubtitleItem *> *)subtitleAtTopItems {
...
// abandon
- (NSDictionary<NSNumber *,PLVVodMediaSubtitleItem *> *)subtitleItemsDictionary {
    if (!_subtitleItemsDictionary) {
        NSMutableDictionary *subtitleItemsDictionary = [NSMutableDictionary dictionary];
        for (PLVVodMediaSubtitleItem *item in self.subtitleItems) {
            subtitleItemsDictionary[@(PLVVodMediaSubtitleTimeGetSeconds(item.startTime))] = item;
        }
...
+ (instancetype)parserWithSubtitle:(NSString *)content error:(NSError *__autoreleasing *)error {
    PLVVodMediaSubtitleParser *parser = [[PLVVodMediaSubtitleParser alloc] init];
    // 字幕文本中，空行带有回车符无法解析
...
- (NSDictionary *)subtitleItemAtTime:(NSTimeInterval)time {
    PLVVodMediaSubtitleItem * item = [self searchSubtitleWithTime:time atTop:NO];
    PLVVodMediaSubtitleItem * itemAtTop = [self searchSubtitleWithTime:time atTop:YES];
...
- (PLVVodMediaSubtitleItem *)searchSubtitleWithTime:(NSTimeInterval)time
                                      atTop:(BOOL)atTop{
    NSMutableArray<PLVVodMediaSubtitleItem *> * arr = atTop ? self.subtitleAtTopItems : self.subtitleItems;
    ...
    while (low <= high) {
        ...
        PLVVodMediaSubtitleItem *thisSub = arr[mid];
        NSTimeInterval thisStartTime = PLVVodMediaSubtitleTimeGetSeconds(thisSub.startTime);
        
        if (thisStartTime <= time) {
            NSTimeInterval thisEndTime = PLVVodMediaSubtitleTimeGetSeconds(thisSub.endTime);
...
#   define SCAN_STRING(str) scanString(scanner, (str))
    ...
    while (![scanner isAtEnd]) {
        ...
        PLVVodMediaSubtitleTime start = { -1, -1, -1, -1 };
        PLVVodMediaSubtitleTime end = { -1, -1, -1, -1 };
        ...
        if (!ok) {
            if (error) {
                ...
                NSDictionary *errorDetail = [NSDictionary dictionaryWithObjectsAndKeys:
                                             ...
                                             nil];
                *error = [NSError errorWithDomain:PLVVodMediaSubtitleErrorDomain
                                            code:-1
        ...
        PLVVodMediaSubtitleItem *item = [[PLVVodMediaSubtitleItem alloc] initWithText:subText
                                                              start:start
                                                                end:end];
