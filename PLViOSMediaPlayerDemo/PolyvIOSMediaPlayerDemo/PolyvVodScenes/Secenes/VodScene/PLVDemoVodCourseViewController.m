//
//  PLVDemoVodCourseViewController.m
...
///  ├── (PLVVodMediaVideoListView) videoListView

@interface PLVDemoVodCourseViewController ()<
PLVVodMediaAreaVCDelegate,
PLVMediaPlayerSkinOutMoreViewDelegate,
PLVVodMediaVideoListViewDelegate
>
...
@property (nonatomic, strong) PLVVodMediaVideoListView *videoListView;
@property (nonatomic, strong) PLVVodMediaVideoListVM *videoListVM;

@end

@implementation PLVDemoVodCourseViewController
...
- (void)viewDidLoad {
    ...
    // Request Video List
    [self.videoListVM requestVideoListWithCompletion:^(NSArray<PLVVodMediaVideo *> *videoList, NSError *error) {
        if (error) {
            NSLog(@"Request video list failed: %@", error);
        } else {
            [self.videoListView updateWithVideoList:videoList];
        }
    }];
}
...
- (PLVVodMediaVideoListView *)videoListView{
    if (!_videoListView){
        _videoListView = [[PLVVodMediaVideoListView alloc] init];
        _videoListView.delegate = self;
    }
    return _videoListView;
}

- (PLVVodMediaVideoListVM *)videoListVM{
    if (!_videoListVM){
        _videoListVM = [[PLVVodMediaVideoListVM alloc] init];
    }
    return _videoListVM;
}
...
- (void)updateUIForOrientation{
    ...
    if (isFullScreen){ // 横向-全屏
        ...
        CGFloat videoListViewY = CGRectGetMaxY(self.vodMediaAreaVC.view.frame);
        self.videoListView.frame = CGRectMake(0, videoListViewY, self.view.bounds.size.width, self.view.bounds.size.height - videoListViewY);
    }
}

#pragma mark - PLVVodMediaVideoListViewDelegate
- (void)vodMediaVideoListView_didSelectVideo:(PLVVodMediaVideo *)video {
    // 更新当前视频ID
    self.vid = video.vid;
    // 播放新视频
    [self.vodMediaAreaVC playWithVid:video.vid];
    // 更新下载按钮状态
    [self initDownloadModule];
}
