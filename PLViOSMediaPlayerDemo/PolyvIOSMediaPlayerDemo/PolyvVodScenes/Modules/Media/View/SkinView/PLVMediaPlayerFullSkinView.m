...
@interface PLVMediaPlayerFullSkinView()
...
@implementation PLVMediaPlayerFullSkinView

@synthesize titleLabel = _titleLabel;

- (void)layoutSubviews{
    BOOL fullScreen = [UIScreen mainScreen].bounds.size.width > [UIScreen mainScreen].bounds.size.height;
    NSLog(@"----- PLVMediaPlayerFullSkinView :FullScreen :%d --- \n", fullScreen);
...
- (void)lockSceenButtonClick:(UIButton *)button{
    //
    if (self.fullSkinDelegate && [self.fullSkinDelegate respondsToSelector:@selector(mediaPlayerFullSkinView_LockSceenEvent:)]){
        [self.fullSkinDelegate mediaPlayerFullSkinView_LockSceenEvent:self];
    }
}

- (void)playRateButtonClick:(UIButton *)button{
    if (self.fullSkinDelegate && [self.fullSkinDelegate respondsToSelector:@selector(mediaPlayerFullSkinView_SwitchPlayRate:)]){
        [self.fullSkinDelegate mediaPlayerFullSkinView_SwitchPlayRate:self];
    }
}

- (void)qualityButtonClick:(UIButton *)button{
    if (self.fullSkinDelegate && [self.fullSkinDelegate respondsToSelector:@selector(mediaPlayerFullSkinView_SwitchQuality:)]){
        [self.fullSkinDelegate mediaPlayerFullSkinView_SwitchQuality:self];
    }
}
